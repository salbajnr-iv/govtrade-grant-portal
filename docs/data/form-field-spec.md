...
| path          | Source route   | Optional | string   | max 120 chars        | `^[A-Za-z0-9_\\-/]+$` | Auto-captured route where issue started. | Non-PII                | Enables first-contact triage and routing.      |
| errorCode     | Error code     | Optional | string   | max 64 chars         | `^[A-Z0-9_\\-]+$`     | Include visible error code if shown.     | Non-PII                | Improves defect clustering + incident routing. |
| requestId     | Request ID     | Optional | string   | max 64 chars         | `^[A-Za-z0-9\\-_.]+$` | Include request ID when available.       | Operational identifier | Correlates case with telemetry/backend logs.   |
| severity      | Severity       | Required | enum     | `low                 | medium                | high                                     | critical`              | -                                              | Choose impact severity. | Non-PII | Controls escalation priority and SLA timer. |
...
| severity      | Invalid enum value         | Select an impact severity level.                        | `VAL_SEVERITY_INVALID`  |
...
| Source route (optional)   | `path`          |
| Error code (optional)     | `errorCode`     |
| Request ID (optional)     | `requestId`     |
| Severity                  | `severity`      |
